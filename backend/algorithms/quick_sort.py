def generate_quick_sort_steps(array: list[int]) -> list[dict]:
    ...
    def partition(left, right, depth) -> int:
        ...
        for j in range(left, right):
            add_step("compare", f"Comparing arr[{j}]={arr[j]} with pivot {pivot}",
                     10, depth=depth, comparing=[j, right],
                     range_val=[left, right], pivot_idx=right, part_idx=i, scan_idx=j)
            stats["comparisons"] += 1

            if arr[j] <= pivot:
                i += 1
                if i != j:
                    add_step("swap", f"Swapping arr[{i}]={arr[i]} and arr[{j}]={arr[j]}",
                             11, depth=depth, swapping=[i, j],
                             range_val=[left, right], pivot_idx=right, part_idx=i, scan_idx=j)
                    arr[i], arr[j] = arr[j], arr[i]
                    stats["swaps"] += 1
                    add_step("swapped", f"Swapped arr[{i}] and arr[{j}]",
                             11, depth=depth, range_val=[left, right],
                             pivot_idx=right, part_idx=i, scan_idx=j)

        add_step("pivot_place", f"Placing pivot {pivot} at index {i + 1}",
                 12, depth=depth, swapping=[i + 1, right],
                 range_val=[left, right], pivot_idx=right, part_idx=i + 1, scan_idx=-1)
    ...
    def quick_sort(left, right, depth):
        ...
        if left == right:
            ...
            add_step("base_case", f"Single element at index {left} is sorted",
                     1, depth=depth, range_val=[left, right])
            add_step("quick_return", f"Returning from range [{left}, {right}]",
                     -1, depth=depth, range_val=[left, right])
            return

        add_step("quick_call", f"Sorting range [{left}, {right}]",
        ...
        add_step("partition_done", f"Pivot placed at index {p}, partitioning [{left},{right}] complete",
